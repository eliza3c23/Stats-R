...
# Part 1a
# Create a histogram of the variable CombFE. Consider this the population. 
hist(CarData.Population$CombFE, main = "Population Vehicles Sold in US", 
     xlab = "Combined Fuel Efficiency (MPG)", col = "lightgreen")
...
N = length(CarData.Population$CombFE)
N

# Calculate the population mean mu
mu = mean(CarData.Population$CombFE)
mu
...
sigma = sd(CarData.Population$CombFE)*sqrt((N-1)/N)
sigma

# Part 1b
# Take a sample of size 40 from the dataset. 
n = 40
CarData.Sample = CarData.Population[sample(1:N,n),]
CarData.Sample

#Create a histogram of the variable CombFE for your sampled data.
hist(CarData.Sample$CombFE, main = "Random Sample of 40 Vehicles Sold in US", 
     xlab = "Combined Fuel Efficiency (MPG)", col = "lightblue")
# Calculate the sample mean for CombFE.
x.bar = mean(CarData.Sample$CombFE)
x.bar

# Calculate the sample standard deviation for combFE.
s = sd(CarData.Sample$CombFE)
s
...
# nsim is the number of times you want to take a sample of 40 from the population.
nsim = 10000
samp.means = rep(0,nsim)

# write a loop that will take a sample of size 40 and from that sample calculate the mean. 
...
# Plots 10000 p-value  
hist(p.values, main = "P-values from 10000 z tests", breaks = seq(0,1,0.05), col = c("red", rep("green",19))) 
